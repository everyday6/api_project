...
| type | 계산 방식 | 산출값 | 규모 |
| :---: | --- | --- | --- |
| 세그먼트별 통과시간 (type1) | 길이 ÷ 가중평균 속도 | 30분 버킷 통과시간(초) | 세그먼트×48버킷, 이론상 최대 약 1,048만 행 |
| 세그먼트별 길이 (type2) | LION 원본 그대로 | 정적 길이값(m) | 세그먼트당 1행, 최대 약 21.8만 행 |
| 세그먼트별 택시 승차수요 (type3) | 최근 N주 평균 → zone→segment 확산 | 요일×30분 슬롯 평균 승차수 | 확산 시 최대 7,300만 건 |
| 세그먼트별 통행료 (type4) | 혼잡통행료 + 도로통행료 합산 | 세그먼트당 통행료 | — (통행료 대상만, 희소) |
