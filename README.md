...
이 프로젝트는 모든 예외를 동일하게 실패 처리하지 않습니다. 데이터 결함은 영향 범위만 격리하고, 일시적인 시스템 장애는 Airflow 재시도 대상으로 남기며, 사용자 요청 경로에서는 정확도를 단계적으로 낮추더라도 응답을 반환하는 것을 우선합니다.

| 구간·상황 | 프로젝트 고유 검증·판정 | 실패 시 처리 | 설계 목적 |
| --- | --- | --- | --- |
| TLC Bronze 필수 컬럼 누락 | Great Expectations로 taxi_type별 원본 필수 컬럼 전체를 `critical` 등급으로 검사 | 해당 Parquet 파일만 처리 대상에서 제외하고 같은 taxi_type의 나머지 파일은 계속 처리 | 파일 하나의 스키마 변경이 월별·차종별 전체 파이프라인 실패로 번지는 것을 방지 |
| TLC Bronze 값 이상 | 빈 데이터, 필수값 NULL, Location ID 범위, 음수 승객 수·거리 등을 `log-only` 등급으로 검사 | 파일은 Silver 단계로 진행하되 실패 항목과 실제 값을 경고 로그로 기록 | 원본 데이터의 현실적인 이상치를 보존하면서 품질 추이를 관찰 |
| S3·Spark·검증 엔진 장애 | 데이터 결함용 예외와 구분해 시스템 예외로 판정 | 예외를 다시 발생시켜 Airflow 재시도와 최종 실패 알림을 실행 | 인프라 장애를 잘못된 파일로 오인해 조용히 제외하지 않음 |
| 다수 파일 동시 제외 | 제외 파일과 사유를 청크가 끝날 때까지 모아 한 번에 요약 | 파일별 개별 알림 대신 청크당 Slack 메시지 1건 전송 | 초기 적재나 원본 스키마 일괄 변경 시 Slack rate limit으로 알림이 유실되는 것을 방지 |
| Silver·Gold 운영 반영 | 실행별 staging에서 필수 컬럼, NULL, 값 범위, 복합키 중복, coverage와 예상 행 수를 먼저 검증 | 검증을 통과한 결과만 운영 테이블에 반영하고 실패 시 기존 정상 데이터 유지 | 부분 생성되거나 불완전한 결과가 서빙 데이터로 노출되는 것을 방지 |
| Type3 시계열 완전성 | `Taxi Zone × 월의 모든 날짜 × 하루 48개 시간대`와 실제 행 수가 정확히 일치하는지 확인하고, 최종 Segment마다 `요일 7개 × 48개 시간대`가 존재하는지 재검증 | 날짜·Zone·시간대 하나라도 누락되거나 복합키가 중복되면 RDS 게시 중단 | 컬럼 형식만 정상인 불완전한 시계열 데이터가 정상 데이터처럼 서빙되는 것을 방지 |
| 참조 데이터 동시 변경 | Zone-Segment 매핑 내용을 정렬해 만든 SHA-256 버전과 롤링 날짜 범위를 게시 직전에 다시 비교 | 계산 계획 이후 매핑 버전이나 날짜 범위가 바뀌었으면 적재 중단 후 다음 실행에서 재계산 | 오래된 매핑으로 계산한 결과와 최신 참조 데이터가 RDS에서 섞이는 경쟁 상태 방지 |
| LION 갱신에 따른 삭제 | 최신 유효 세그먼트와 기존 RDS 행을 안티조인해 삭제 대상을 계산 | 유효 세그먼트 집합이 비어 있으면 삭제하지 않고 예외 발생 | 상류 파이프라인 오류로 RDS 서빙 테이블 전체가 비워지는 사고 방지 |
| Type1 경로 조회값 누락·노후화 | 요청 시각에 앞 세그먼트의 소요시간을 누적해 다음 세그먼트의 실제 도착 버킷을 계산하고, `최신 실측값 → 과거 평균 → 도로 길이·제한속도 기반 추정값` 순으로 조회 | 개별 단계에서 값을 찾지 못하면 다음 정확도 계층으로 전환 | 단순 고정 시간 조회가 아니라 실제 경로 진행 시각에 맞는 값을 최대한 제공 |
| RDS 연결 실패·쿼리 지연 | 짧은 연결·쿼리 타임아웃과 연속 실패 기반 circuit breaker로 장애를 빠르게 감지 | `Lambda 메모리 캐시 → S3 Gold 스냅샷 → 코드 기본값` 순으로 전환하고 남은 세그먼트를 채워 응답 | RDS 장애가 세그먼트 수만큼 누적돼 내비게이션 요청 전체가 타임아웃되는 것을 방지 |

특히 Type1은 첫 세그먼트에만 요청 시각을 그대로 사용합니다. 이후 세그먼트는 앞 구간에서 계산된 소요시간을 누적한 도착 시각으로 RDS의 30분 버킷을 조회합니다. 예를 들어 12:29에 출발해 첫 구간에서 2분이 걸렸다면, 다음 세그먼트는 12:31에 도착한 것으로 보고 12:30 버킷을 사용합니다. 이 과정에서 RDS 장애가 감지되면 남은 경로에 같은 실패를 반복하지 않고 즉시 대체 경로로 전환합니다.
