...
RDS는 정상 응답했지만 이 세그먼트·시간대 값의 신뢰도가 낮을 때(결측/오래됨), 신뢰도 순으로 내려가는 4단계 체인을 둡니다.

| 단계 | 값 | 조건 |
| :---: | --- | --- |
| 1 | 최신 실측값 (Fresh) | `현재 - observed_at ≤ 신선도 기준` |
| 2 | 과거 대표값 (Historical AVG) | Fresh 없거나 오래됨 |
| 3 | 도로 스펙 추정 (Spec Estimate) | AVG도 없음 — `길이 ÷ 제한속도` |
| 4 | 코드 기본값 (Hardcoded) | 스펙도 없음 |
