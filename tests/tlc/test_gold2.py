...
def spark():
    session = SparkSession.builder.master("local[1]").appName("tlc_gold2_test").getOrCreate()
    yield session
    session.stop()


def _write_tlc_silver_fixture(base_dir, taxi_type, month, rows):
    """base_dir/{taxi_type}_tripdata_{month}/data.parquet 형태로 TLC silver1 픽스처를 만든다.

    coerce_timestamps="us"는 이 테스트 환경의 pyarrow(21.x)가 pandas
    datetime64[ns] 컬럼을 기본값으로 Parquet TIMESTAMP(NANOS)로 쓰기 때문에
    필요하다. 실제 운영 TLC silver1 파일은 Spark 자체가 마이크로초 단위로
...
# (구 tests/mapping/test_segment_spatial_weight.py — src/tlc/gold2.py로 흡수)
# =========================================================
...
def test_match_points_to_zone_assigns_zone_id(monkeypatch):
    ...
    monkeypatch.setattr("src.tlc.gold2._load_zones", lambda path: zones)
