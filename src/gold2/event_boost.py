...
1. "진앙" 레코드 = event_lion(도로 통제형 행사, mapping_status=="matched"만)
   + ticketmaster_lion(공연장, 버퍼/최근접 매칭, segment_id 있는 행 전부) 둘
   다 합쳐서 만든다.
   ...
   - ticketmaster는 관객 수 등 규모 정보가 없어 전부 동일 가중치(보수적으로
     event보다 낮게). end_ts가 있는 경우가 거의 없어서(실측 약 2%), 없으면
...
# TODO(팀 검토 필요): 근거 없는 초안 — 관객 수 등 규모 정보가 없어 이벤트
# 하나당 동일 가중치로 취급(도로를 아예 막는 event보다는 보수적으로 낮게).
...
def load_ground_zero_records(event_mapping_dt: str, ticketmaster_mapping_dt: str) -> pd.DataFrame:
    """
    event_lion + ticketmaster_lion 매핑 결과를 합쳐 segment_id x start_ts x
    ...
    tm = db.read_partition(
        MAP_TICKETMASTER_LION_TABLE,
        ticketmaster_mapping_dt,
        columns=["segment_id", "start_ts", "end_ts"],
    )
