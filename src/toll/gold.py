...
결합해서 최종 segment_id -> value를 만들고 RDS(segment_metrics_type4)에
쓴다. 혼잡통행료와 도로통행료는 원래 type=4/5로 나눠서 저장했는데, 한
segment가 CBD zone 안이면서 동시에 다리/터널 시설이기도 한 경우(zone
진입 지점의 다리 segment 등) 클라이언트가 매번 두 타입을 다 조회해서
더해야 했다. "택시가 이 segment를 지나는 데 드는 통행료 총액"이 실제로
필요한 값이므로, 여기서 미리 합산해 type=4(TYPE_TOLL) 하나로 합쳤다 —
nav-gold 전체 설계(시간=1/길이=2/수요=3/통행료=4)의 4타입 구성과도 맞다.

통행료는 시간대에 따라 안 바뀌므로(택시 정액 요금 — 스펙의 Global
Constraints 참고) type1처럼 시간 버킷을 저장하지 않는다 - segment_id당
행 하나뿐이다(src/common/rds.py의 ensure_static_table 참고).
"""
...
def build_gold_items(rate_table: dict, zone_map: pd.DataFrame, facility_map: pd.DataFrame) -> list[dict]:
    """혼잡통행료 + 도로통행료를 segment_id별로 합산해 RDS 행 하나로
...
def write_gold_items(items: list[dict]) -> None:
    # ensure_static_table은 원래 로컬/테스트 편의용이라 운영 경로에서는 안
    # 쓰는 게 원칙이지만, 이 파이프라인은 처음부터 그렇게 짜여 있었고
    # 여기서 빼면 배포 순서에 새로 의존하게 되어 그대로 유지한다.
    # idempotent라 반복 호출해도 안전.
    rds.ensure_static_table(RDS_TABLE_TYPE4)
    count = rds.upsert_static_items(items, RDS_TABLE_TYPE4)
    logger.info(f"[toll_gold] RDS에 {count}개 아이템 적재 완료")
