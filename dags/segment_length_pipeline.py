...
LION 원본 다운로드/정제(dim_segment)는 lion_pipeline이 담당한다 - 예전엔
이 파이프라인도 독자적으로 같은 LION zip을 받아 같은 dim_segment.parquet를
만들었는데(중복), 이제 lion_pipeline이 dim_segment를 발행할 때 emit하는
lion_dim_segment_ready Asset에 반응해서 그 결과를 그대로 읽어 RDS 반영만
한다(is_routable 같은 별도 가공 단계 없음 - 2026-08-26 제거, LION 세그먼트는
전부 서빙 대상으로 취급한다).

LION 파싱은 이제 필요 없어 GDAL 의존이 사라졌고, 세그먼트당 값 하나
(10~30만 행)뿐이라 EMR Serverless를 쓸 만큼 큰 데이터가 아니다 - Airflow
워커 안에서 pandas로 직접 처리한다(2026-08-25 Spark/EMR -> pandas 전환.
write_to_rds/to_serving_items는 애초에 결과를 driver로 collect해서 순수
파이썬으로 처리하고 있었어서, 실질적으로는 이미 pandas나 다름없었다).
...
아니라 filter_valid_length_segments가 걸러낸 "유효 ID"다 - length_ft가
0 이하로 바뀌면 Type1 대상에서도 빠져야 하기 때문이다(Type2와 동일한
필터를 그대로 재사용).
"""
...
def segment_length_pipeline():
    ...
    def write_gold2_to_rds(dim_segment_path: str) -> int:
        ...
        filtered = filter_valid_length_segments(df)
    ...
    def cleanup_type1_stale_segments(dim_segment_path: str) -> dict:
        ...
        입력받아 같은 필터(filter_valid_length_segments)로 "지금 유효한 ID"를
        ...
        영향은 미미하다고 보지만(막 유효 대상에서 빠진 세그먼트는 대체로
        실제 GPS 트래픽도 같이 없다), 엄격히 막으려면 두 파이프라인을
        같은 1-slot Airflow pool로 직렬화해야 한다(후속 과제)."""
        df = pd.read_parquet(dim_segment_path)
        valid_ids = filter_valid_length_segments(df)["segment_id"].tolist()
        result = db.cleanup_keys_not_in(SERVING_TABLE_TYPE1, valid_ids, "segment_id")
        return {**result, "stale_keys": result["stale_keys"][:20]}

    dim_segment_path = str(DIM_SEGMENT_BASE_PATH)
    write_gold2_to_rds(dim_segment_path)
    cleanup_type1_stale_segments(dim_segment_path)
