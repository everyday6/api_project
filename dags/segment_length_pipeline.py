...
속도 데이터가 아니라 LION 도로 스펙만 씀)도 여기서 같이 처리한다 - "하나의
...
from src.common.config import DYNAMODB_TABLE_TYPE1, DYNAMODB_TABLE_TYPE2, EMR_JOBS_DIR, PROJECT_ROOT
...
from src.nav_time.gold2 import compute_spec_travel_seconds, spec_estimate_items, write_to_dynamodb
...
@dag(
    ...
    description="type2(길이) + type1 SPEC Estimate — LION 정적 스펙을 DynamoDB에 upsert (lion_pipeline Asset 트리거)",
...
def segment_length_pipeline():
    ...
    def build_and_write_spec_estimates(dim_segment_path: str) -> int:
        ...
        return write_to_dynamodb(items, DYNAMODB_TABLE_TYPE1)
