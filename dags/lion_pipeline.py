...
분기 단위로 나오는 새 릴리즈를 놓치지 않도록 매일 변경 여부를 확인한다
...
매일 확인이나 재시도/수동 재실행이 겹쳐도 원본이 그대로면 다운로드,
...
with DAG(
    ...
    description="LION(도로망) 매일 변경 확인 및 Bronze/Silver1",
    schedule="0 4 * * *",            # 매일 새벽 4시(KST)
    start_date=pendulum.datetime(2025, 1, 1, tz="Asia/Seoul"),
    catchup=False,                    # 과거 확인은 채우지 않음 (최신 버전이면 충분)
    max_active_runs=1,
    render_template_as_native_obj=True,
    default_args=default_args,
    tags=["lion", "quarterly-source", "daily-check"],
) as dag:
